...
# import win32ui
# import win32con
framer = cv.imread('White.jpg')

def home(request):
    list_of_openwindows = gw.getAllTitles()
    l = []
    colorlist = list(colorstext.keys())
    langlist = list(lang.keys())
    print(list_of_openwindows)
    for i in range(len(list_of_openwindows)):
        if list_of_openwindows[i] == 'Settings':
            break
        if list_of_openwindows[i] not in l:
            l.append(list_of_openwindows[i])
    context = {
        'list_of_openwindows': l,
        'colorlist': colorlist,
        'langlist': langlist,
    }
    print(l)
...
def run(request):
    windowTile = request.GET['windowtile']
    colorTile = request.GET['textcolor']
    langdet = request.GET['langdet']
    if langdet != 'auto':
        langdet = lang[langdet]
    langtrans = lang[request.GET['langtrans']]
    print(request.GET['windowtile'])
    with mss.mss() as sct:
        loop_time = time.time()
        while(True):
            window = gw.getWindowsWithTitle(windowTile)[0]
            x = window.left 
            y = window.top + 220
            w = window.width
            h = window.height - 250
            ...
            for i in fin_dict["text"]:
                translated.append(GoogleTranslator(source=langdet, target=langtrans).translate(i))
